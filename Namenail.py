...
def rename_and_move_images(base_name="image", generate_thumbs=False, thumb_size=200, input_path="input", output_path="output", thumb_path=None):
    ...
    if not os.path.isdir(input_path):
        print(f"Input folder not found. Creating new folder at: {input_path}")
...
def launch_gui():
    ...
    root = tk.Tk()
    root.title("Namenail GUI")
    root.configure(padx=30, pady=30)

    tk.Label(root, text="Prefix for renamed images:").grid(row=0, column=0)
    ...
    tk.Checkbutton(root, text="Generate thumbnails", variable=thumb_var).grid(row=3, column=0, columnspan=3)
    ...
    tk.Label(root, text="Thumbnails folder:").grid(row=5, column=0)
    ...
    tk.Button(root, text="Run", command=on_submit).grid(row=6, column=0, columnspan=3, pady=10)

    console = tk.Text(root, height=10, width=80, state='disabled', bg="#0f0f0f")
    console.grid(row=7, column=0, columnspan=3, pady=(10, 0))
